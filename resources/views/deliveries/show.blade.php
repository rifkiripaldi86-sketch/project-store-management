...
<style>
    ...
    .item-product-name small {
        display: block; font-size: 11.5px; color: var(--ink-muted); font-weight: 400; margin-top: 2px;
    }
    ...
    /* Delete confirm */
    .btn-danger-outline {
        height: 38px; font-size: 13.5px; border-radius: var(--r-sm);
        padding: 0 16px; border: 1px solid #fecaca;
...
<div class="page-header animate-in">
    ...
    <div>
        ...
        <p>Kiriman dari {{ $delivery->supplier->nama_supplier }}</p>
    </div>
</div>
...
<div class="form-card">
    <div class="form-card-header">
        ...
        <div class="form-card-header-text">
            <h5>Informasi Kiriman #{{ $delivery->id }}</h5>
            <p>Diinput oleh {{ $delivery->createdBy->name }}
                &mdash; {{ \Carbon\Carbon::parse($delivery->tanggal)->translatedFormat('d F Y') }}
            </p>
        </div>
    </div>
    ...
    <div class="form-body">
        ...
        <div class="info-grid">
            <div class="info-field">
                ...
                <div class="info-field-value">
                    ...
                    {{ \Carbon\Carbon::parse($delivery->tanggal)->translatedFormat('d F Y') }}
                </div>
            </div>
            <div class="info-field">
                <span class="info-field-label">Supplier</span>
                <div class="info-field-value">
                    <i class="fas fa-building" style="font-size:12px; color:var(--ink-muted); margin-right:8px;"></i>
                    {{ $delivery->supplier->nama_supplier }}
                </div>
            </div>
            <div class="info-field">
                <span class="info-field-label">Diinput Oleh</span>
                <div class="info-field-value">
            ...
            <div class="info-field">
                <span class="info-field-label">Jumlah Produk</span>
                <div class="info-field-value">
        ...
        @foreach($delivery->items as $item)
            @php
                $subtotal = $item->jumlah_kirim * $item->harga;
                $grandTotal += $subtotal;
            @endphp
            <div class="item-row">
                ...
                <div class="item-cell">
                    {{ number_format($item->jumlah_kirim, 0, ',', '.') }}
                    <small style="display:inline; color:var(--ink-muted);">unit</small>
                </div>
    ...
    <div class="form-footer">
        ...
        <form method="POST" action="{{ route('deliveries.destroy', $delivery) }}"
            ...
            <button type="button" class="btn-danger-outline" onclick="confirmDelete()">
                <i class="fas fa-trash-can" style="font-size:12px;"></i> Hapus Kiriman
            </button>
        </form>

        <span style="margin-left:auto; font-size:12px; color:var(--ink-muted);">
            ID Kiriman: <strong>#{{ $delivery->id }}</strong>
        </span>
    </div>
</div>
...
function confirmDelete() {
    if (confirm('Hapus kiriman ini? Stok produk akan dikurangi kembali sesuai jumlah kiriman.')) {
